...
if ( ! class_exists( 'CMB2_Bootstrap_224_Trunk', false ) ) {
	...
	class CMB2_Bootstrap_224_Trunk {
		...
		public function l10ni18n() {
			...
			$loaded = load_plugin_textdomain( 'tattoostudio', false, '/languages/' );

			if ( ! $loaded ) {
				$loaded = load_muplugin_textdomain( 'tattoostudio', '/languages/' );
			}

			if ( ! $loaded ) {
				$loaded = load_theme_textdomain( 'tattoostudio', get_stylesheet_directory() . '/languages/' );
			}

			if ( ! $loaded ) {
				$locale = apply_filters( 'plugin_locale', get_locale(), 'tattoostudio' );
				$mofile = dirname( __FILE__ ) . '/languages/cmb2-' . $locale . '.mo';
				load_textdomain( 'tattoostudio', $mofile );
			}
