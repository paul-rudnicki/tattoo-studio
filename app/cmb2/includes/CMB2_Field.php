...
class CMB2_Field extends CMB2_Base {
	...
	public function _set_field_defaults( $args ) {
		...
		$args['options']    = 'group' == $args['type'] ? wp_parse_args( $args['options'], array(
			'add_button'    => esc_html__( 'Add Group', 'tattoostudio' ),
			'remove_button' => esc_html__( 'Remove Group', 'tattoostudio' ),
		) ) : $args['options'];
		...
		if ( in_array( $args['type'], $option_types, true ) ) {
			...
			$args['show_option_none'] = true === $args['show_option_none'] ? esc_html__( 'None', 'tattoostudio' ) : $args['show_option_none'];

			if ( null === $args['show_option_none'] ) {
				$off_by_default = in_array( $args['type'], array( 'select', 'radio', 'radio_inline' ), true );
				$args['show_option_none'] = $off_by_default ? false : esc_html__( 'None', 'tattoostudio' );
			}
	...
	public function get_cmb() {
		if ( ! $this->cmb_id ) {
			return new WP_Error( 'no_cmb_id', esc_html__( 'Sorry, this field does not have a cmb_id specified.', 'tattoostudio' ) );
		}
